...
<div id="contain">
<div id="app">
<legend><div class="header">เพิ่มชมรม</div></legend><br>
<form method="post" class="form-horizontal">
	<label class="control-label" for="club">ชื่อชมรม : </label>
    <div class="controls"><input type="text" name="club" required></div><br>
...
<?php
require 'header.php';
$club = htmlspecialchars($_POST['club'] ,ENT_QUOTES);
...
if($club != NULL )
{
	...
	$sql = mysqli_query($status,"select club from data_club where club = '$club'");
	$empty = mysqli_fetch_array($sql);
	if(empty($empty)){
		$sql = mysqli_query($status,"select count(*) from data_club");
		$count = mysqli_fetch_array($sql);
		$sql = ("insert into data_club(club,limit_member,teacher,des,condition_m,condition_s_m,condition_s_fm) values('$club','$limit','$name','$describe','$string_condition_m','$male','$female')");
		mysqli_query($status,$sql) or die("Error Sql Database!" . mysqli_error());
		echo '<script>alert("เพิ่มชมรมสำเร็จ");</script>';
	}
	else{
		echo '<script>alert("ชื่อชมรมนี้ได้ถูกใช้แล้ว โปรดใช้ชื่อใหม่");</script>';
	}
}
?>
</body>
